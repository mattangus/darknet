...
namespace kernel
{
    ...
    __global__ void elementwise(T* data, size_t n, T other)
    {
        CUDA_1D_KERNEL_LOOP(idx, n)
        {
            data[idx] = F()(data[idx], other);
        }
    }
...
namespace gpu
{
    ...
    {
        auto grid = get_number_of_blocks(n);
        kernel::elementwise<T, F><<<grid, BLOCK>>>(data, n, other);
        // TODO: for some reason the macro doesn't compile. need to fix that.
        if (cudaPeekAtLastError() != cudaSuccess)
            throw std::runtime_error(cudaGetErrorString(cudaPeekAtLastError()));
    }

    #define MAKE_TYPES(TYPE, FNC) template void elementwise<TYPE, FNC>(TYPE* data, size_t n, TYPE other);
    #define MAKE_ALL(TYPE)  MAKE_TYPES(TYPE, plus<TYPE>); \
                            MAKE_TYPES(TYPE, minus<TYPE>); \
                            MAKE_TYPES(TYPE, divides<TYPE>); \
                            MAKE_TYPES(TYPE, multiplies<TYPE>);

    NUMERIC_TYPES(MAKE_ALL);
    ...
    #define MAKE_TYPES(TYPE, FNC) template void elementwise<TYPE, FNC>(TYPE* data, size_t n, TYPE* other, size_t m);
    #define MAKE_ALL(TYPE)  MAKE_TYPES(TYPE, plus<TYPE>) \
                            MAKE_TYPES(TYPE, minus<TYPE>) \
                            MAKE_TYPES(TYPE, divides<TYPE>) \
                            MAKE_TYPES(TYPE, multiplies<TYPE>)
